...
import { DfMcpExposureComponent } from 'src/app/adf-ai/mcp/df-mcp-exposure/df-mcp-exposure.component';
import { DfMcpCreateComponent } from 'src/app/adf-ai/mcp/df-mcp-create/df-mcp-create.component';
import {
  SYSTEM_MCP_TOOLS,
  SystemMcpTool,
  isSystemMcpType,
...
export class DfServiceDetailsComponent implements OnInit {
  ...
  // ---- MCP server page (df-mcp-exposure). The exposure grid, modes strip
  // and catalog replaced the synthesised per-service tool panels; the host
  // only hands over the config form group and the shared disabledTools set.

  /** Config controls the MCP page owns; hidden from the generic Options loop. */
  private static readonly MCP_OWNED_FIELDS = new Set([
    'exposedServices',
    'disabledTools',
    'lazyMode',
    'toolStyle',
  ...
  isMcpOwnedField(name: string): boolean {
    return this.isMcp && DfServiceDetailsComponent.MCP_OWNED_FIELDS.has(name);
  }

  get mcpConfigGroup(): FormGroup {
    return this.serviceForm.get('config') as FormGroup;
  }

  /** exposed_services.values from the type schema (attachable backends). */
  get mcpAttachableNames(): string[] | null {
    const field = this.configSchema?.find(c => c.name === 'exposedServices');
    return Array.isArray(field?.values)
      ? (field?.values as Array<{ name: string }>).map(v => v.name)
      : null;
  }

  get mcpCustomToolNames(): string[] {
  ...
  get mcpCreateMode(): boolean {
    return (
      this.isMcp && !this.edit && this.serviceForm.getRawValue().type === 'mcp'
    );
  }

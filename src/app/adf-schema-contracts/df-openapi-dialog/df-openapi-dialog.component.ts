...
@Component({
    ...
    <div mat-dialog-content class="openapi-dialog">
      ...
      <ng-container *ngIf="response && !loading">
        <section class="header-row">
          <div class="meta">
            <div><strong>Schema name:</strong> <code>{{ response.schemaName }}</code></div>
          </div>
        ...
        <p class="hint" *ngIf="response.source === 'live'">
          Generated from live schema. Lock the table (or set mode to auto/strict)
          to freeze this contract.
        </p>
  ...
  `,
  styles: [`
    .openapi-dialog { min-width: 600px; max-height: 75vh; overflow: auto; }
    ...
    .source-badge {
      ...
      &.locked { background: #e8f5e9; color: #1b5e20; }
    }
...
export class DfOpenApiDialogComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.contracts.getTableOpenApi(this.data.service, this.data.table)
      .pipe(catchError(err => {
  ...
  copy(): void {
    ...
    navigator.clipboard?.writeText(this.schemaJson).then(
      () => this.snackBar.open('Schema copied to clipboard', 'Dismiss', { duration: 2500 }),

...
export class DfAnalyticsService {
  ...
  private fetchStats(): Observable<DashboardStats> {
    ...
    const requests = {
      services: this.http.get<any>('/api/v2/system/service?fields=id,name,type&include_count=true'),
      roles: this.http.get<any>('/api/v2/system/role?fields=id,name&include_count=true'),
  ...
  private transformResponses(responses: any): DashboardStats {
    ...
    const systemServiceNames = [
      'system', 'api_docs', 'files', 'logs', 'db', 'email', 'user', 'script',
      'ui', 'schema', 'api_doc', 'file', 'log', 'admin', 'df-admin', 
      'dreamfactory', 'cache', 'push', 'pub_sub'
    ].map(s => s.toLowerCase());
